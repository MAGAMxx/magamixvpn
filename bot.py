...
async def send_main_menu(event: Message | CallbackQuery, user_name: str):
    text = (
        f"Привет, {user_name} 👋\n\n"
...
async def tarif_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        f"Последний штрих, и ты почувствуешь ⚡ скорость!\n\n"
        f"Оплата:\nНомер: 79283376737\nБанк: ОЗОН БАНК\nСумма: {price}₽\n\n"
...
async def free_3days(callback: CallbackQuery, state: FSMContext):
    text = (
        "Хочешь 3 дня бесплатно? 🎁\n\n"
        f"Подпишись на канал {CHANNEL_USERNAME} и нажми «Проверить подписку»"
    )
    kb = [
        [InlineKeyboardButton(text="📢 Подписаться на канал", url=CHANNEL_LINK)],
        [InlineKeyboardButton(text="✅ Проверить подписку", callback_data="check_free_sub")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_main")]
    ]
    await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def check_free_sub(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if chat_member.status in ("member", "administrator", "creator"):
            deeplink = create_hiddify_user(3, f"Free_{user_id}")
            if deeplink:
                await callback.message.edit_text(
                    f"🎉 Подписка подтверждена!\nТвои 3 дня бесплатно:\n{deeplink}\n\nУстанови Happ и наслаждайся!"
                )
                await bot.send_message(ADMIN_ID, f"Бесплатные 3 дня выданы: {callback.from_user.full_name} (ID: {user_id})")
            else:
        ...
        await callback.answer("Ошибка проверки. Убедись, что бот — админ канала.", show_alert=True)
    await state.clear()

# Установка VPN
@dp.callback_query(F.data == "install")
async def install(callback: CallbackQuery):
    # Пока берём последнюю подписку (в будущем — список из БД)
    # Для теста — создаём заглушку, если нужно реальную — добавь БД
    text = (
...
async def back_main(callback: CallbackQuery):
    await send_main_menu(callback, callback.from_user.first_name)
...
async def get_screenshot(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Чек отправлен админу. Ожидай!", reply_markup=main_menu())
    await state.clear()
